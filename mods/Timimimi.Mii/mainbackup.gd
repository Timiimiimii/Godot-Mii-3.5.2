...
func MiiHeadAssign(player: Actor):
	...
	if player.name == "player":
		...
		if file.file_exists(str(CharinfoPathKnown+str(playerSteamID)+".charinfo")) == true:
			...
			#CalcArmatureScaleTest(player,MiiInstance.MiiHeight,MiiInstance.MiiWeight)
		elif CharinfoArray.is_empty()==false:
...
func CalcArmatureScaleTest(player, MiiHeight, MiiWeight):
	...
	scale.z = scale.x
	var HeadBoneTransform = player.get_node_or_null("body/player_body/Armature/Skeleton3D").get_bone_rest(11).scaled(Vector3(1,1,1)/scale)
	var HeadScale
	var HeadBoneTransformP = player.get_node_or_null("body/player_body/Armature/Skeleton3D").get_bone_pose(11).scaled(Vector3(1,1,1)/scale)
	...
	#arml
	player.get_node_or_null("body/player_body/Armature/Skeleton3D").set_bone_rest(3,CalcBoneScaleR(player,scale,3))

	#armr
	player.get_node_or_null("body/player_body/Armature/Skeleton3D").set_bone_rest(7,CalcBoneScaleR(player,scale,7))
	#legl
	player.get_node_or_null("body/player_body/Armature/Skeleton3D").set_bone_rest(13,CalcBoneScaleR(player,scale,13))
	#legr
	player.get_node_or_null("body/player_body/Armature/Skeleton3D").set_bone_rest(16,CalcBoneScaleR(player,scale,16))
...
func CalcBoneScaleR(player,scale,bone):
	var BoneTransform = player.get_node_or_null("body/player_body/Armature/Skeleton3D").get_bone_rest(int(bone))
	BoneTransform = BoneTransform.scaled(scale)
	return BoneTransform
func CalcBoneScaleP(player,scale,bone):
	var BoneTransform = player.get_node_or_null("body/player_body/Armature/Skeleton3D").get_bone_pose(int(bone)).scaled(scale)
	return BoneTransform

func AddMiiToInv():
	var InvBut = load("res://mods/Timimimi.Mii/Assets/Node/Misc/mii_inv_button.tscn").instantiate()
	PlayerHUD.get_node_or_null("main/menu/tabs/outfit/HBoxContainer").add_child(InvBut)
	
func NewPlayerMii(MiiFile):
	var MiiHead = get_player_node().get_node_or_null("body/player_body/Armature/Skeleton3D/face").get_child(0)
	MiiHead.MiiFileLoad(str(CharinfoPathOther)+str(MiiFile))

func _on_mii_cont_ready():
	var MiiIcon = $SubViewport/MiiHead
