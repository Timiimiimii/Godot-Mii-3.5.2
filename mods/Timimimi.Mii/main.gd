...
var MiiHead = load("res://mods/Timimimi.Mii/Assets/Mii/node/mii_head_plus.tscn")
var player
var latest_scene_name = ""
#Viewport/main/map/main_map/zones/main_zone/NPCs
...
var NPCPos = Vector3(53.754,3.216,-62.791)
var entities
var MiiNPC = preload("res://mods/Timimimi.Mii/Assets/Node/Misc/ChangeMiiNPC.tscn")
var test = false
var PlayerAPI
var CharinfoArray = []
var CatHeadMesh
...
func MiiHeadAssign(player: Actor):
	...
	if player.name == "player":
		...
		PlayerMiiHead = MiiInstance.get_node(".")
		...
		if CharinfoArray.is_empty() == false:
			...
			elif file.file_exists(str(CharinfoPathKnown) + str(playerSteamUsername) + ".charinfo") == true:
				MiiInstance.MiiFileLoad(str(CharinfoPathKnown) + str(playerSteamUsername) + ".charinfo")
			else :
				MiiInstance.MiiFileLoad(str(CharinfoPathOther) + str(CharinfoArray[CharInfoSelectedNo]))
