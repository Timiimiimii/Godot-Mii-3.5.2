...
onready var MiiHead = $MiiBody/Mii/Skeleton/BoneAttachment/player_face
...
func _ready():
	...
	$MiiBody/Mii/Skeleton/Shirt.get_surface_material(0).albedo_color = MiiHead.FavColour
	$MiiBody/Mii/Skeleton/Pants.get_surface_material(0).albedo_color = Color("40464e")
	CharinfoArrayMenu = MiiMod.GrabMiiFiles(CharinfoPathOther)
	if (MiiSelectGUI):
		MiiSelectMenu = MiiSelectGUI.instance()
		MiiSelectMenu.hide()
