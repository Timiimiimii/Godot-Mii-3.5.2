...
func init_player(player: Actor):
	if player.name == "player":
		PlayerMiiHead = player.get_node("body/player_body/Armature/Skeleton/face").get_child(0)
...
func init_miiButton():
	...
	for Mii in MiiList:
		...
		var ButInstance = MiiButton.instance()
		MiiCont.add_child(MiiButton.instance())
		...
		ButInstance.connect("_MiiSelected", self, "_EditPlayerMii")
